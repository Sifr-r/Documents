...
## 2. Install and Configure Prisma (part 2)
...
## 3. Integrate Prisma into SolidStart (part 2)
      ...
      <For each={users() ?? []}> // [!code ++]
        {(user) => ( // [!code ++]
          <div> // [!code ++]
            ...
            <For each={user.posts}>{(post) => <p>{post.title}</p>} // [!code ++]
          </div> // [!code ++]
        )} // [!code ++]
       // [!code ++]
    </main>
        ...
        <Show when={!users.error} fallback={<p>Error loading data</p>}> // [!code ++]
          <For each={users()}>
            {(user) => (
              <div>
                <h3>{user.name}</h3>
                <For each={user.posts}>{(post) => <p>{post.title}</p>}
              </div>
            )}
