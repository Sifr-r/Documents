# Embedded Prisma Studio (with Next.js)
...
[**Embeddable Prisma Studio**](/studio/integrations/embedding) is _free_ and licensed under Apache 2.0.
...
## 2. Setting up Prisma ORM and Prisma Postgres (part 2)
...
```typescript title="prisma/seed.ts" tab="Prisma Postgres (PostgreSQL)"
...
```typescript title="prisma/seed.ts" tab="SQLite"
import { PrismaClient } from "../app/generated/prisma/client";
...
```typescript title="prisma/seed.ts" tab="MySQL"
import { PrismaClient } from "../app/generated/prisma/client";
import { PrismaMySQL } from "@prisma/adapter-mysql";

## 2. Setting up Prisma ORM and Prisma Postgres (part 3)
...
```ts title="prisma.config.ts" tab="Prisma Postgres (PostgreSQL)"
...
```ts title="prisma.config.ts" tab="SQLite"
import "dotenv/config";
import { defineConfig, env } from "prisma/config";
export default defineConfig({
  schema: "prisma/schema.prisma",
...
```ts title="prisma.config.ts" tab="MySQL"
import "dotenv/config";
import { defineConfig, env } from "prisma/config";
export default defineConfig({
  schema: "prisma/schema.prisma",
...
```typescript title="app/api/studio/route.ts" tab="PostgreSQL"
import "dotenv/config";
import { createPrismaPostgresHttpClient } from "@prisma/studio-core/data/ppg";
import { serializeError } from "@prisma/studio-core/data/bff";

## 3. Setting up the embedded Prisma Studio in your app (part 2)
...
```typescript title="app/api/studio/route.ts" tab="SQLite"
import "dotenv/config";
...
## 3. Setting up the embedded Prisma Studio in your app (part 3)
...
```typescript title="app/api/studio/route.ts" tab="MySQL"
import "dotenv/config";
...
```tsx title="app/page.tsx" tab="PostgreSQL"
"use client";
...
## 3. Setting up the embedded Prisma Studio in your app (part 4)
...
```tsx title="app/page.tsx" tab="SQLite"
"use client";
...
```tsx title="app/page.tsx" tab="MySQL"
"use client";
...
## 3. Setting up the embedded Prisma Studio in your app (part 5)
