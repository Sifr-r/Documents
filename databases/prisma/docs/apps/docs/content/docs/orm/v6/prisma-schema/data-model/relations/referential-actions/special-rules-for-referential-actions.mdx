...
model Egg {
  id         Int       @id @default(autoincrement())
  predator   Fox       @relation(fields: [predatorId], references: [id]) // [!code --]
  predator   Fox       @relation(fields: [predatorId], references: [id], onUpdate: NoAction) // [!code ++]
  predatorId Int
  parents    Chicken[]
}
```
...
model Fox {
  id        Int     @id @default(autoincrement())
  meal      Chicken @relation(fields: [mealId], references: [id]) // [!code --]
  meal      Chicken @relation(fields: [mealId], references: [id], onUpdate: NoAction) // [!code ++]
  mealId    Int
  foodStore Egg[]
}
```
...
model Comment {
  ...
  postId      Int
  writtenBy   User @relation(fields: [writtenById], references: [id])
  post        Post @relation(fields: [postId], references: [id]) // [!code --]
  post        Post @relation(fields: [postId], references: [id], onUpdate: NoAction) // [!code ++]
}
```

or

## Multiple cascade paths between two models (SQL Server only) (part 2)
...
model Post {
  ...
  author   User      @relation(fields: [authorId], references: [id], onUpdate: NoAction) // [!code ++]
  comments Comment[]
}
```
