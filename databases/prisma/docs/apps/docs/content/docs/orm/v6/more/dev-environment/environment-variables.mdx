...
An environment variable is a key value pair of string data that is stored on your machine's local environment. Refer to our [Environment variables reference documentation](/orm/v6/reference/environment-variables-reference) for specific details.
...
With Prisma ORM v6.4.0, we released the `prisma.config.ts` file. This file allows you to manage your environment variables and settings in a more flexible way. [View our reference](/orm/v6/reference/prisma-config-reference) for more information.
...
## How Prisma ORM can use environment variables (part 3)
...
When running tests, we advise you to [mock Prisma Client](/orm/v6/prisma-client/testing/unit-testing#mocking-prisma-client). In doing so, you need to tell Jest which environment it should use when running its tests.
