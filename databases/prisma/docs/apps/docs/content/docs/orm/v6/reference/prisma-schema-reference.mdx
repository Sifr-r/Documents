...
| `url`               | **Yes**  | String (URL)                                                                    | **Deprecated in Prisma ORM v7.** Configure the connection URL in Prisma Config instead: see [`datasource.url`](/orm/v6/reference/prisma-config-reference#datasourceurl). Existing schemas continue to work, but you should migrate to Prisma Config.                                                                                            |
| `shadowDatabaseUrl` | No       | String (URL)                                                                    | **Deprecated in Prisma ORM v7.** Configure the shadow database URL in Prisma Config instead: see [`datasource.shadowDatabaseUrl`](/orm/v6/reference/prisma-config-reference#datasourceshadowdatabaseurl).                                                                                                                                       |
| `directUrl`         | No       | String (URL)                                                                    | **Deprecated in Prisma ORM v7.** Configure the direct connection URL in Prisma Config instead: see [`datasource.directUrl`](/orm/v6/reference/prisma-config-reference#datasourcedirecturl-removed).                                                                                                                                             |
| `relationMode`      | No       | String (`foreignKeys`, `prisma`)                                                | Sets whether [referential integrity](/orm/v6/prisma-schema/data-model/relations/relation-mode) is enforced by foreign keys in the database or emulated in the Prisma Client.<br /><br />In preview in versions 3.1.1 and later. The field is named `relationMode` in versions 4.5.0 and later, and was previously named `referentialIntegrity`. |
| `extensions`        | No       | List of strings (PostgreSQL extension names)                                    | Allows you to [represent PostgreSQL extensions in your schema](/orm/v6/prisma-schema/postgresql-extensions). Available in preview for PostgreSQL only in Prisma ORM versions 4.5.0 and later.                                                                                                                                                   |
...
- [`sqlserver`](/orm/v6/overview/databases/sql-server)
...
Learn more about PostgreSQL connection strings [here](/orm/v6/overview/databases/postgresql).
...
Learn more about MySQL connection strings [here](/orm/v6/overview/databases/mysql).
...
Learn more about MongoDB connection strings [here](/orm/v6/overview/databases/mongodb).
...
Learn more about SQLite connection strings [here](/orm/v6/overview/databases/sqlite).
...
The format for connection strings is the same as for PostgreSQL. Learn more about PostgreSQL connection strings [here](/orm/v6/overview/databases/postgresql).

## generator

Defines a [generator](/orm/v6/prisma-schema/overview/generators) in the Prisma schema.

### Fields for `prisma-client-js` provider

This is the default generator for Prisma ORM 6.x and earlier versions. Learn more about [generators](/orm/v6/prisma-schema/overview/generators#prisma-client-js-deprecated).

A `generator` block accepts the following fields:

| Name              | Required | Type                         | Description                                                                                                                                                                                                        |
| :---------------- | :------- | :--------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `provider`        | **Yes**  | `prisma-client-js`           | Describes which [generator](/orm/v6/prisma-schema/overview/generators) to use. This can point to a file that implements a generator or specify a built-in generator directly.                                 |
| `output`          | No       | String (file path)           | Determines the location for the generated client, [learn more](/orm/v6/prisma-client/setup-and-configuration/generating-prisma-client#using-a-custom-output-path). **Default**: `node_modules/.prisma/client` |
...
| `engineType`      | No       | Enum (`library` or `binary`) | Defines the [query engine](/orm/v6/more/internals/engines) type to download and use. **Default**: `library`                                                                                              |
| `binaryTargets`   | No       | List of Enums (see below)    | Specify the OS on which the Prisma Client will run to ensure compatibility of the [query engine](/orm/v6/more/internals/engines). **Default**: `native`                                                  |
...
The ESM-first client generator that offers greater control and flexibility across different JavaScript environments. It generates plain TypeScript code into a custom directory, providing full visibility over the generated code. Learn more about the new [`prisma-client`](/orm/v6/prisma-schema/overview/generators#prisma-client) generator.
...
| Name                     | Required | Type                                                                                                               | Description                                                                                                                                                                                                                 |
| :----------------------- | :------- | :----------------------------------------------------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `provider`               | **Yes**  | `prisma-client`                                                                                                    | Describes which [generator](/orm/v6/prisma-schema/overview/generators) to use. This can point to a file that implements a generator or specify a built-in generator directly.                                          |
| `output`                 | **Yes**  | String (file path)                                                                                                 | Determines the location for the generated client, [learn more](/orm/v6/prisma-client/setup-and-configuration/generating-prisma-client#using-a-custom-output-path).                                                     |
| `previewFeatures`        | No       | List of Enums                                                                                                      | Use intellisense to see list of currently available Preview features (`Ctrl+Space` in Visual Studio Code) **Default**: none                                                                                                 |
...
The following tables list all supported operating systems with the name of platform to specify in [`binaryTargets`](/orm/v6/prisma-schema/overview/generators#binary-targets).
...
Defines a Prisma [model](/orm/v6/prisma-schema/data-model/models#defining-models) .
...
[Fields](/orm/v6/prisma-schema/data-model/models#defining-fields) are properties of models.
...
> **Note**: You can use the [`@map` attribute](#map) to [map a field name to a column](/orm/v6/prisma-client/setup-and-configuration/custom-model-and-field-names) with a different name that does not match field naming conventions: e.g. `myField @map("my_field")`.
...
Prisma models also have [model field types](/orm/v6/prisma-schema/data-model/relations) that define relations between models.
...
| `xml`                | `@db.Xml`                      |
| `inet`               | `@db.Inet`                     |
| `citext`             | `@db.Citext`                   | Only available if [Citext extension is enabled](/orm/v6/prisma-schema/data-model/unsupported-database-features#enable-postgresql-extensions-for-native-database-functions). |
...
## model field scalar types (part 4)
...
## model field scalar types (part 5)
...
| :--------------- | :-------------------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------- |
| Prisma Client JS | [`BigInt`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/BigInt) | See [examples of working with `BigInt`](/orm/v6/prisma-client/special-fields-and-types#working-with-bigint) |
...
## model field scalar types (part 6)
...
## model field scalar types (part 7)
...
| Client           | Type                                               | Description                                                                                                        |
| :--------------- | :------------------------------------------------- | :----------------------------------------------------------------------------------------------------------------- |
| Prisma Client JS | [`Decimal`](https://mikemcl.github.io/decimal.js/) | See [examples of working with `Decimal`](/orm/v6/prisma-client/special-fields-and-types#working-with-decimal) |
...
You can find more info and examples in this section: [Working with `DateTime`](/orm/v6/prisma-client/special-fields-and-types#working-with-datetime).
...
## model field scalar types (part 8)
...
## model field scalar types (part 9)
...
## model field scalar types (part 10)
...
| Client                                                                                                    | Type                                                                                                        | Description                                                                                                    |
| :-------------------------------------------------------------------------------------------------------- | :---------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------- |
| Prisma Client JS                                                                                          | [`Uint8Array`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Uint8Array) | See [examples of working with `Bytes`](/orm/v6/prisma-client/special-fields-and-types#working-with-bytes) |
| Prisma Client JS ([before v6](/orm/v6/more/upgrades/to-v7)) | [`Buffer`](https://nodejs.org/api/buffer.html)                                                              | See [examples of working with `Bytes`](/orm/v6/prisma-client/special-fields-and-types#working-with-bytes) |
...
The [MongoDB connector](/orm/v6/overview/databases/mongodb) does not support the `Unsupported` type.
...
## model field scalar types (part 11)
...
```prisma highlight=3;normal tab="Relational databases"
model User {
  id             Int      @id @default(autoincrement())
...
```prisma highlight=3;normal tab="MongoDB"
model User {
...
In this scenario, you cannot create a `Profile` only - you must use Prisma Client's [nested writes](/orm/v6/prisma-client/queries/relation-queries#nested-writes) create a `User` **or** connect the profile to an existing user.
...
**Not supported by MongoDB** <br />
The [MongoDB connector](/orm/v6/overview/databases/mongodb) does not support composite IDs.
...
Defines a [default value for a field](/orm/v6/prisma-schema/data-model/models#defining-a-default-value).

#### Remarks

- Default values that cannot yet be represented in the Prisma schema are represented by the [`dbgenerated()` function](#dbgenerated) when you use [introspection](/orm/v6/prisma-schema/introspection).
...
- Default values can be used with [scalar lists](/orm/v6/prisma-client/special-fields-and-types/working-with-scalar-lists-arrays) in databases that natively support them.
...
Defines a compound [unique constraint](/orm/v6/prisma-schema/data-model/models#defining-a-unique-field) for the specified fields.
...
- In version `3.12.0` and later, you can define an index on a field of a [composite type](/orm/v6/prisma-schema/data-model/models#defining-composite-types) using the syntax `@@index([compositeType.field])`. See [Defining composite type indexes](/orm/v6/prisma-schema/data-model/models#defining-composite-type-indexes) for more details.
...
Defines meta information about the relation. [Learn more](/orm/v6/prisma-schema/data-model/relations#the-relation-attribute).
...
| `fields`     | `FieldReference[]`                                                                                                                                     | On [annotated](/orm/v6/prisma-schema/data-model/relations#relation-fields) relation fields | A list of [fields](/orm/v6/prisma-schema/data-model/models#defining-fields) of the _current_ model                                                                            | `["authorId"]`, `["authorFirstName, authorLastName"]` |
| `references` | `FieldReference[]`                                                                                                                                     | On [annotated](/orm/v6/prisma-schema/data-model/relations#relation-fields) relation fields | A list of [fields](/orm/v6/prisma-schema/data-model/models#defining-fields) of the model on _the other side of the relation_                                                  | `["id"]`, `["firstName, lastName"]`                   |
| `map`        | `String`                                                                                                                                               | No                                                                                              | Defines a [custom name](/orm/v6/prisma-schema/data-model/database-mapping#constraint-and-index-names) for the foreign key in the database.                                    | `["id"]`, `["firstName, lastName"]`                   |
| `onUpdate`   | Enum. See [Types of referential actions](/orm/v6/prisma-schema/data-model/relations/referential-actions#types-of-referential-actions) for values. | No                                                                                              | Defines the [referential action](/orm/v6/prisma-schema/data-model/relations/referential-actions) to perform when a referenced entry in the referenced model is being updated. | `Cascade`, `NoAction`                                 |
...
See: [The `@relation` attribute](/orm/v6/prisma-schema/data-model/relations#the-relation-attribute).
...
> See [Using custom model and field names](/orm/v6/prisma-client/setup-and-configuration/custom-model-and-field-names) to see how `@map` and `@@map` changes the generated Prisma Client.
...
In versions before [4.4.0](https://github.com/prisma/prisma/releases/tag/4.4.0), if you're also using [`now()`](/orm/v6/reference/prisma-schema-reference#now), the time might differ from the `@updatedAt` values if your database and app have different time zones. This happens because `@updatedAt` operates at the Prisma ORM level, while `now()` operates at the database level.
...
```prisma line-number tab="Relational databases"
model Post {
  id        String   @id
...
```prisma line-number tab="MongoDB"
model Post {
  id        String   @id @map("_id") @db.ObjectId
  updatedAt DateTime @updatedAt
}
...
Add `@@schema` to a model to specify which schema in your database should contain the table associated with that model. Learn more about [adding multiple schema's here](/orm/v6/prisma-schema/data-model/multi-schema).

:::note

[Multiple database schema](/orm/v6/prisma-schema/data-model/multi-schema) support is only available with the PostgreSQL, CockroachDB, and SQL Server connectors.
...
For more information about using the `multiSchema` feature, refer to [this guide](/orm/v6/prisma-schema/data-model/multi-schema).
...
**Not supported by MongoDB** <br />
The [MongoDB connector](/orm/v6/overview/databases/mongodb) does not support the `autoincrement()` function.
...
The sequence function is only supported by [CockroachDB connector](/orm/v6/overview/databases/cockroachdb).
...
- Compatible with `String`.
- Implemented by Prisma ORM and therefore not "visible" in the underlying database schema. You can still use `uuid()` when using [introspection](/orm/v6/prisma-schema/introspection) by [manually changing your Prisma schema](/orm/v6/prisma-client/setup-and-configuration/custom-model-and-field-names) and [generating Prisma Client](/orm/v6/prisma-client/setup-and-configuration/generating-prisma-client), in that case the values will be generated by Prisma ORM's [query engine](/orm/v6/more/internals/engines).
...
[`gen_random_uuid()` is a PostgreSQL function](https://www.postgresql.org/docs/13/functions-uuid.html). To use it in PostgreSQL versions 12.13 and earlier, you must enable the `pgcrypto` extension.<br /><br />In Prisma ORM versions 4.5.0 and later, you can declare the `pgcrypto` extension in your Prisma schema with the [`postgresqlExtensions` preview feature](/orm/v6/prisma-schema/postgresql-extensions).
...
The [Microsoft SQL Server connector](/orm/v6/overview/databases/sql-server) does not support the `enum` type.

:::

Defines an [enum](/orm/v6/prisma-schema/data-model/models#defining-enums) .
...
Defines a [composite type](/orm/v6/prisma-schema/data-model/models#defining-composite-types) .
