...
## Raw queries with relational databases (part 4)
...
## SQL injection prevention (part 2)
...
## Raw queries with MongoDB (part 2)
...
```ts no-lines
<model>.aggregateRaw(args?: {pipeline?: InputJsonObject[], options?: InputJsonObject}): PrismaPromise;
```
