...
- The `$transaction` API in Prisma Client:
  - [Sequential operations](#sequential-prisma-client-operations): pass an array of Prisma Client queries to be executed sequentially inside a transaction, using `$transaction(queries: PrismaPromise[]): Promise<R[]>`.
...
- [Sequential operations](#sequential-prisma-client-operations): Pass an array of Prisma Client queries to be executed sequentially inside of a transaction.
  ...
  `$transaction(queries: PrismaPromise[]): Promise<R[]>`
...
const createTeamOperation = prisma.team.create({
  data: {
    ...
    members: {
      connect: {
        id: createUserOperation.id, // Not possible, ID not yet available // [!code --]
      },
    },
  },
});
