...
```prisma title="schema.prisma" highlight=10,16-17;normal showLineNumbers
model User {
  id    Int    @id @default(autoincrement())
  posts Post[]
}
...
## What are referential actions? (part 2)
...
## Types of referential actions (part 2)
...
## Types of referential actions (part 3)
...
## Types of referential actions (part 4)
...
model Post {
  id       Int    @id @default(autoincrement())
  title    String
  author   User   @relation(fields: [authorId], references: [id], onDelete: Cascade) // [!code ++]
  authorId Int
}
...
## Upgrade paths from versions 2.25.0 and earlier (part 2)
...
## Upgrade paths from versions 2.25.0 and earlier (part 3)
...
async function main() {
  try {
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      if (error.code === 'P2014') { // [!code --]
      if (error.code === 'P2003') { // [!code ++]
        console.log(error.message)
      }
    }
  }
