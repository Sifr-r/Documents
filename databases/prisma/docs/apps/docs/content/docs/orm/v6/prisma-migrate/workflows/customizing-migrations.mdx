...
1. Edit the draft migration as shown, changing `DROP` / `DELETE` to a single `RENAME COLUMN`:
   ...
   ```sql title="./prisma/migrations/20210308092620_rename_migration/migration.sql" tab="Before"
   ALTER TABLE "Profile" DROP COLUMN "biograpy",
   ADD COLUMN  "biography" TEXT NOT NULL;
   ```

   ```sql title="./prisma/migrations/20210308092620_rename_migration/migration.sql" tab="After"
   ALTER TABLE "Profile"
...
## How to edit a migration file (part 2)
...
## How to edit a migration file (part 3)
