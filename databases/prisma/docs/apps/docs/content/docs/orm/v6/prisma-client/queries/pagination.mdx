...
![Diagram illustrating offset pagination: skipping the first 3 records and returning records 4 through 7 using skip and take.](/img/orm/offset-skip-take.png)
...
![Diagram showing the first page of cursor-based pagination with 4 results, where the cursor (ID 29) marks the last record for the next query.](/img/orm/cursor-1.png)
...
![Diagram showing cursor-based pagination results after the cursor (ID 29), with the new cursor at ID 52.](/img/orm/cursor-2.png)
...
![Diagram showing cursor pagination without skip: 1, where the result set includes the previous cursor record.](/img/orm/cursor-3.png)
