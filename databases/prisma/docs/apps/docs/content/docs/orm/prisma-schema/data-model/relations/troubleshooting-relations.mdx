...
The `@relation` annotation can also be used to [name the underlying relation table](/orm/prisma-schema/data-model/relations/many-to-many-relations#configuring-relation-table-name) created on a implicit many-to-many relationship.
