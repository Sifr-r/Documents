...
```prisma highlight=4,5;normal tab="Relational databases"
model Post {
  id       Int       @id @default(autoincrement())
  // Other fields
...
```prisma highlight=4,5;normal tab="MongoDB"
model Post {
  id       String    @id @default(auto()) @map("_id") @db.ObjectId
  // Other fields
  comments Comment[] // A list of comments // [!code highlight]
...
```prisma highlight=3,5;normal tab="MongoDB"
model Post {
  id         String     @id @default(auto()) @map("_id") @db.ObjectId
...
```prisma highlight=10;normal tab="Relational databases"
model Post {
  id         Int        @id @default(autoincrement())
  createdAt  DateTime   @default(now())
  title      String
...
```prisma highlight=10;normal tab="MongoDB"
model Post {
  id         String     @id @default(auto()) @map("_id") @db.ObjectId
  createdAt  DateTime   @default(now())
  title      String
...
```prisma highlight=5,8-11;normal tab="MongoDB"
model User {
  id    String  @id @default(auto()) @map("_id") @db.ObjectId
  email String  @unique
  name  String?
...
## Models in Prisma Client (part 2)
