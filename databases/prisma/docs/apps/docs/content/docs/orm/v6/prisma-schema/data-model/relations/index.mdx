...
```prisma highlight=3,8,9;normal tab="Relational databases"
model User {
...
```prisma highlight=3,8,9;normal tab="MongoDB"
model User {
...
![A one-to-many relationship between a user and posts table.](/img/v6/orm/prisma-schema/data-model/relations/one-to-many.png)
...
![The sample schema as an entity relationship diagram](/img/v6/orm/prisma-schema/data-model/relations/sample-schema.png)
...
```prisma highlight=6,7,13,15;normal no-copy tab="Relational databases"
...
```prisma highlight=6,7,13,15;normal no-copy tab="MongoDB"
// NOTE: This schema is intentionally incorrect. See below for a working solution.
...
```prisma highlight=4,5,11,13;normal tab="Relational databases"
model User {
  id           Int     @id @default(autoincrement())
  name         String?
...
```prisma highlight=4,5,11,13;normal tab="MongoDB"
model User {
  id           String  @id @default(auto()) @map("_id") @db.ObjectId
  name         String?
  writtenPosts Post[]  @relation("WrittenPosts") // [!code highlight]
...
## Disambiguating relations (part 2)
