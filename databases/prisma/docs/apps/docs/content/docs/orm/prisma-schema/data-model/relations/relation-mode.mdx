...
model Post {
  ...
  title    String
  author   User   @relation(fields: [authorId], references: [id], onDelete: Cascade, onUpdate: Cascade) // [!code highlight]
...
-- AddForeignKey
ALTER TABLE "Post" -- [!code highlight]
  ADD CONSTRAINT "Post_authorId_fkey" -- [!code highlight]
  FOREIGN KEY ("authorId") -- [!code highlight]
  REFERENCES "User"("id") ON DELETE CASCADE ON UPDATE CASCADE; -- [!code highlight]
```
...
```prisma title="schema.prisma" highlight=4,9;add showLineNumbers
datasource db {
  provider     = "mysql"
...
- `prisma`: this emulates relations in Prisma Client. You should also [enable this option](/orm/core-concepts/supported-databases/mysql#planetscale) when you use the MySQL connector with a PlanetScale database and don't have native foreign key constraints enabled in your PlanetScale database settings.
...
The `foreignKeys` relation mode is not available when you use the MongoDB connector. Some relational databases, [such as PlanetScale](/orm/core-concepts/supported-databases/mysql#planetscale), also forbid the use of foreign keys. In these cases, you should instead [emulate relations in Prisma ORM with the `prisma` relation mode](#emulate-relations-in-prisma-orm-with-the-prisma-relation-mode).
...
## Emulate relations in Prisma ORM with the prisma relation mode (part 2)
...
```prisma title="schema.prisma" highlight=6;add showLineNumbers
model Post {
  id     Int  @id
  userId Int
  user   User @relation(fields: [userId], references: [id])
...
## Emulate relations in Prisma ORM with the prisma relation mode (part 3)

If you use the [Prisma VS Code extension](https://marketplace.visualstudio.com/items?itemName=Prisma.prisma) (or our [language server in another editor](/orm/more/dev-environment/editor-setup)), the warning is augmented with a Quick Fix that adds the required index for you:

![The Quick Fix pop-up for adding an index on a relation scalar field in VS Code](/img/orm/prisma-schema/data-model/relations/quick-fix-index.png)
