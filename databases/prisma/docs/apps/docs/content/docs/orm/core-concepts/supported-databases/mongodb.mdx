...
- [Add to existing MongoDB project](/prisma-orm/add-to-existing-project/mongodb)
...
- model your database with the [Prisma Schema Language](/orm/prisma-schema/overview)
- connect to your database, using the [`mongodb` database connector](/orm/core-concepts/supported-databases)
...
- use [Prisma Client](/orm/prisma-client/setup-and-configuration/introduction) in your application to query your database in a type safe way based on your Prisma Schema
...
Further reference information is available in the [MongoDB connector documentation](/orm/core-concepts/supported-databases/mongodb).
...
To connect to a MongoDB server, configure the [`datasource`](/orm/prisma-schema/overview/data-sources) block in your [Prisma Schema](/orm/prisma-schema/overview):
...
- `url`: Specifies the [connection URL](#connection-url) for the MongoDB server. In this case, an [environment variable is used](/orm/more/dev-environment/environment-variables) to provide the connection URL.
...
![Structure of the MongoDB connection URL](/img/orm/core-concepts/databases/mongodb.png)
...
The [`@@id`](/orm/reference/prisma-schema-reference) attribute (an ID for multiple fields) is not supported because primary keys in MongoDB are always on the `_id` field of a model.
...
See [Special rules for referential actions](/orm/prisma-schema/data-model/relations/referential-actions#special-rules-for-sql-server-and-mongodb) for more details.
