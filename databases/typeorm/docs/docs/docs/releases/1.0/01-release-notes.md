...
- **MySQL: `QueryBuilder.useIndex()` accepts index name(s), not raw SQL** — `useIndex()` now takes either a single index name or an array of names and escapes them as identifiers. Previously the argument was interpolated as a raw, comma-separated SQL fragment, which allowed identifier injection ([#12344](https://github.com/typeorm/typeorm/pull/12344) by [@eddieran](https://github.com/eddieran))
...
- **`LegacyOracleNamingStrategy` and `NamingStrategyV03` moved to `@typeorm/legacy-naming-strategies`** — these classes are no longer exported from `typeorm` core; install the separate `@typeorm/legacy-naming-strategies` package and import them from there if you still need them ([#12416](https://github.com/typeorm/typeorm/pull/12416) by [@alumni](https://github.com/alumni))
...
- **SAP HANA: generated columns** — `@Column({ generatedType: "STORED", asExpression: "..." })` is now supported in SAP HANA ([#12393](https://github.com/typeorm/typeorm/pull/12393) by [@Cprakhar](https://github.com/Cprakhar))
...
- **`JsonContains` accepts array values** — `JsonContains([...])` no longer throws and produces the expected `@>` containment predicate on PostgreSQL ([#12420](https://github.com/typeorm/typeorm/pull/12420) by [@kyungseopk1m](https://github.com/kyungseopk1m))
- **MSSQL computed columns join scope** — schema introspection of computed columns is now scoped to the correct table when multiple tables share column names ([#12288](https://github.com/typeorm/typeorm/pull/12288) by [@PreAgile](https://github.com/PreAgile))
...
- **User-defined shared join columns preserved in change set** — explicit `@JoinColumn` entries shared between relations are no longer dropped during persistence ([#12354](https://github.com/typeorm/typeorm/pull/12354) by [@PreAgile](https://github.com/PreAgile))
...
- **PostgreSQL: sequential query execution** — queries are now executed sequentially on the same connection to avoid `pg` 8.19.0 deprecation warnings, including remaining relation-load and persistence paths ([#12105](https://github.com/typeorm/typeorm/pull/12105) by [@pkuczynski](https://github.com/pkuczynski), [#12421](https://github.com/typeorm/typeorm/pull/12421) by [@kyungseopk1m](https://github.com/kyungseopk1m))
...
- **PostgreSQL / CockroachDB: enum value declaration order** — enum values are now loaded in their declared order, so generated migrations no longer churn when nothing has changed ([#12404](https://github.com/typeorm/typeorm/pull/12404) by [@pkuczynski](https://github.com/pkuczynski))
- **Expo: auto-load `expo-sqlite`** — the Expo driver loads `expo-sqlite` through `loadDependencies()`, removing the need to pass the module explicitly ([#12363](https://github.com/typeorm/typeorm/pull/12363) by [@pkuczynski](https://github.com/pkuczynski))


### Other

- **CLI `init` command** — no longer crashes when `package.json` does not exist ([#11947](https://github.com/typeorm/typeorm/pull/11947) by [@gioboa](https://github.com/gioboa)); published package now correctly includes `devDependencies` needed for scaffolding ([#12281](https://github.com/typeorm/typeorm/pull/12281) by [@pkuczynski](https://github.com/pkuczynski)); no longer runs `npm install` during init, so it works in offline or non-Node project setups ([#12386](https://github.com/typeorm/typeorm/pull/12386) by [@alumni](https://github.com/alumni))
...
- **Query stack traces preserved for MySQL/MSSQL** — driver errors now retain the originating call site, making query failures easier to debug ([#12056](https://github.com/typeorm/typeorm/pull/12056) by [@Cprakhar](https://github.com/Cprakhar))
...
- **`.limit()` validated on Update/SoftDelete query builders** — non-numeric values passed to `.limit()` now throw at call time instead of being interpolated into SQL ([#12436](https://github.com/typeorm/typeorm/pull/12436) by [@smith-xyz](https://github.com/smith-xyz))
