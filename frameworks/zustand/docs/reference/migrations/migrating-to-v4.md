...
- create:
-   < State
-   , StoreSetState = StoreApi["set"]
-   , StoreGetState = StoreApi["get"]
-   , Store = StoreApi
-   >
-     (f: ...) => ...
+ create:
...
Else, replace `create<T, ...>(...)` with `create()(...)`.
...
Replace `PartialState<T, ...>` with `PartialState`
...
Replace `UseBoundStore` with `UseBoundStore<StoreApi>`,
and `UseBoundStore<T, S>` with `UseBoundStore`
...
```diff
  createContext:
-   <State, Store = StoreApi>() => ...
+   () => ...
```

**Migration**

Replace `createContext()` with `createContext<StoreApi>()`,
and `createContext<T, S>()` with `createContext()`.
...
The type of partialized state is now `T` instead of `Partial`,
...
If your partialized state is truly `Partial`,
