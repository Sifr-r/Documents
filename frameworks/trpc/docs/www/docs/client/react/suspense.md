...
- If you use suspense with [tRPC's _automatic_ SSR in Next.js](/docs/client/nextjs/pages-router/ssr), the full page will crash on the server if a query fails, even if you have an ``
...
const trpcClient = createTRPCClient({ links: [httpBatchLink({ url: 'http://localhost:3000' })] });
