...
<InstallSnippet pkgs="@trpc/server@^11 @trpc/client@^11 @trpc/react-query@^11 @trpc/next@^11 @tanstack/react-query@^5 @tanstack/react-query-devtools@^5" />
...
See the [subscriptions docs](../server/subscriptions.md#stopping-from-server) for more information.

### Added support for lazy-loading routers (non-breaking)

See the [lazy-loading routers docs](../server/merging-routers.md#lazy-load) for more information.
...
See the [standalone adapter docs](../server/adapters/standalone.md#custom-basePath) for more information.
...
## Full reverse-chronological changelog (part 2)
...
```ts twoslash
// @target: esnext
type inferAsyncIterableYield =
  TOutput extends AsyncGenerator<infer $Yield> ? $Yield : never;
```



## Full reverse-chronological changelog (part 3)
...
## Full reverse-chronological changelog (part 4)
...
### Added `inferProcedureBuilderResolverOptions`-helper (non-breaking)
...
### Transformers are moved to links (breaking) - {#transformers-moved}
...
## Full reverse-chronological changelog (part 5)
...
### Deleted `inferHandlerInput` and `ProcedureArgs` (non-breaking for most)

> If these types mean nothing for you or your codebase, just ignore this

Use `inferProcedureInput` instead & `TRPCProcedureOptions` instead
...
### React Query peerDep is now v5 (breaking) - {#react-query-v5}
...
## Full reverse-chronological changelog (part 6)
...
### `interop`-mode has been removed (rarely breaking) - {#interop-mode-removed}
