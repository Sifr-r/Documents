...
````yaml /langsmith/langsmith-platform-openapi.json post /v1/platform/alerts/{session_id}/test
openapi: 3.1.0
info:
  title: LangSmith
  description: >+
...
tags:
  ...
  - name: skills
    x-hidden: true
paths:
  /v1/platform/alerts/{session_id}/test:
    post:
      tags:
        - alert_rules
      summary: Test an alert action to determine if configuration is valid
      description: >-
        Tests an alert action which will fire a notification to all configured
        recipients if the configuration is valid.
      parameters:
      ...
      responses:
        '200':
          description: Alert action fired successfully
          content:
            application/json:
              schema:
