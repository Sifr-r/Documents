...
````yaml /langsmith/managed-deep-agents-openapi.json delete /agents/{agent_id}
openapi: 3.1.0
info:
...
paths:
  /agents/{agent_id}:
    delete:
      ...
      description: >-
        Delete the agent. The call is idempotent: deleting a non-existent agent
        returns `204`. Deletion does not cascade to the agent's threads —
        existing threads remain queryable but cannot start new runs (attempts
        return `502`). Delete threads explicitly when you want to clean them up.
