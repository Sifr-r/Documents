...
````yaml /langsmith/managed-deep-agents-openapi.json post /agents/{agent_id}/clone
openapi: 3.1.0
info:
...
paths:
  /agents/{agent_id}/clone:
    post:
      ...
      description: >-
        Create a new agent that mirrors the source agent behavior but is owned
        by the caller. The clone copies runtime, backend, file tree, tools,
        subagents, and skills; caller metadata and sharing state start fresh.
      operationId: cloneManagedDeepAgent
      parameters:
        - name: agent_id
          in: path
          required: true
        ...
        - name: include_files
          ...
          required: false
          description: >-
            Include the raw file map in the agent response. Accepted truthy
        ...
        - name: X-User-Timezone
          ...
          required: false
          description: >-
            IANA timezone name (for example, `America/Los_Angeles`) stamped into
            the agent's runtime configuration when the agent is created or
            updated. The runtime uses this timezone to inject a localized
            current-date line into the agent's system prompt, so the agent
            reasons about dates in the user's local time. Optional. Defaults to
            `UTC` when omitted; invalid values fall back to `UTC`.
          schema:
            type: string
  ...
  schemas:
    CloneAgentRequest:
      ...
      properties:
        name:
          type: string
          description: Optional clone name override.
        permissions:
