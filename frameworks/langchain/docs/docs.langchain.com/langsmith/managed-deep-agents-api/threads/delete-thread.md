...
````yaml /langsmith/managed-deep-agents-openapi.json delete /threads/{thread_id}
openapi: 3.1.0
info:
...
paths:
  /threads/{thread_id}:
    delete:
      ...
      description: >-
        Delete a thread and all of its runs. Idempotent: deleting a missing
        thread returns 204.
      operationId: deleteManagedDeepAgentThread
      parameters:
        - name: thread_id
          in: path
          required: true
