...
````yaml /langsmith/managed-deep-agents-openapi.json get /threads/{thread_id}
openapi: 3.1.0
info:
  ...
  /threads/{thread_id}:
    get:
      tags:
        - Threads
      summary: Get thread
      description: Return thread metadata and status for the given thread ID.
      operationId: getManagedDeepAgentThread
      parameters:
