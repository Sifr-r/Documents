...
````yaml /langsmith/managed-deep-agents-openapi.json patch /threads/{thread_id}
openapi: 3.1.0
info:
...
paths:
  /threads/{thread_id}:
    patch:
      ...
      description: >-
        Update thread fields such as title and metadata. Omitted fields are
        unchanged.
      operationId: updateManagedDeepAgentThread
      parameters:
        - name: thread_id
          in: path
          required: true
