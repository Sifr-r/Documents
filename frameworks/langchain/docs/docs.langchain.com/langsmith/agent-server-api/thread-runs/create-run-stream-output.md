...
````yaml /langsmith/agent-server-openapi.json post /threads/{thread_id}/runs/stream
openapi: 3.1.0
info:
...
For each key specified, a unique URL will be generated that can be
        ...
        if_not_exists:
          ...
          title: If Not Exists
          description: >-
            How to handle missing thread. Must be either 'reject' (raise error
            if missing), or 'create' (create new thread).
          default: reject
        after_seconds:
          type: number
          title: After Seconds
