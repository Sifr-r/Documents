...
- "local" modules exist within a Nuxt project, either [inlined in Nuxt config](/docs/3.x/api/nuxt-config#modules) or within [the `modules` directory](/docs/3.x/directory-structure/modules).
...
You can get type hinting for this function using the higher-level `defineNuxtModule` helper provided by [Nuxt Kit](/docs/3.x/guide/going-further/kit).
...
::tip
Learn more about asset injection in [the recipes section](/docs/3.x/guide/modules/recipes-basics).
::
