...
Enables the experimental [``](/docs/3.x/api/components/nuxt-client-fallback) component for rendering content on the client if there's an error in SSR.
...
Enables experimental component islands support with [``](/docs/3.x/api/components/nuxt-island) and `.island.vue` files.
...
Out of the box, this will enable typed usage of [`navigateTo`](/docs/3.x/api/utils/navigate-to), [``](/docs/3.x/api/components/nuxt-link), [`router.push()`](/docs/3.x/api/composables/use-router) and more.
