...
## Delayed (or Lazy) Hydration (part 2)
...
Server-only components use [``](/docs/3.x/api/components/nuxt-island) under the hood, meaning that `lazy` prop and `#fallback` slot are both passed down to it.
...
If an island needs information about the current route, pass it in explicitly &mdash; either as props from the parent component, or via the `context` prop on `` (read inside the island from `nuxtApp.ssrContext.islandContext`).
::
