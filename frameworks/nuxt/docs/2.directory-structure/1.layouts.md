...
Layouts are enabled by adding [``](/docs/3.x/api/components/nuxt-layout) to your [`app.vue`](/docs/3.x/directory-structure/app):
...
You can directly override the default layout for all pages using the `name` property of [``](/docs/3.x/api/components/nuxt-layout):
...
::important
If you use `` within your pages, make sure it is not the root element (or [disable layout/page transitions](/docs/3.x/getting-started/transitions#disable-transitions)).
::
