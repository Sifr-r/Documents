...
If you are using [`app.vue`](/docs/3.x/directory-structure/app), make sure to use the [``](/docs/3.x/api/components/nuxt-page) component to display the current page:
...
It is possible to display [nested routes](https://router.vuejs.org/guide/essentials/nested-routes) with ``.
...
Nuxt will automatically wrap your page in [the Vue `` component](https://vuejs.org/guide/built-ins/keep-alive#keepalive) if you set `keepalive: true` in your `definePageMeta`. This might be useful to do, for example, in a parent route that has dynamic child routes, if you want to preserve page state across route changes.

When your goal is to preserve state for parent routes use this syntax: `<NuxtPage keepalive />`. You can also set props to be passed to `` (see [a full list](https://vuejs.org/api/built-in-components#keepalive)).
...
To navigate between pages of your app, you should use the [``](/docs/3.x/api/components/nuxt-link) component.
