...
```ts [Signature]
export function defineNuxtPlugin<T extends Record<string, unknown>> (plugin: Plugin | ObjectPlugin): Plugin & ObjectPlugin

type Plugin = (nuxt: NuxtApp) => Promise<void> | Promise<{ provide?: T }> | void | { provide?: T }
...
| `setup`     | `Plugin`{lang="ts"}                 | `false`  | The main plugin function, equivalent to a function plugin.                                                                                                          |
| `hooks`     | `Partial`{lang="ts"} | `false`  | Nuxt app runtime hooks to register directly.                                                                                                                        |
