...
interface PageMeta {
  validate?: ((route: RouteLocationNormalized) => boolean | Promise<boolean> | Partial | Promise<Partial>)
...
**Experimental feature, only available when [enabled in your nuxt.config file](/docs/3.x/getting-started/transitions#view-transitions-api-experimental)**</br>
  ...
  **`validate`**
  ...
  - **Type**: `(route: RouteLocationNormalized) => boolean | Promise<boolean> | Partial | Promise`
