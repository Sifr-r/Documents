...
```ts [Signature]
export function useHeadSafe (input: MaybeComputedRef): void
```
...
`input`: A `MaybeComputedRef` object containing head data. You can pass all the same values as [`useHead`](/docs/3.x/api/composables/use-head), but only safe attributes will be rendered.
