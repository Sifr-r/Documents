...
## Migrating to Vitest 3.0 {#vitest-3} (part 2)
...
### `Custom` Type is Deprecated <Badge type="danger">API {#custom-type-is-deprecated}
...
### The `WorkspaceSpec` Type is No Longer Used <Badge type="danger">API {#the-workspacespec-type-is-no-longer-used}
...
### Changes to the Snapshot API <Badge type="danger">API {#changes-to-the-snapshot-api}
...
## Migrating to Vitest 3.0 {#vitest-3} (part 3)

### Changes to `resolveConfig` Type Signature <Badge type="danger">API {#changes-to-resolveconfig-type-signature}
...
### Cleaned up `vitest/reporters` types <Badge type="danger">API {#cleaned-up-vitest-reporters-types}
...
### Simplified Generic Types of Mock Functions (e.g. `vi.fn`, `Mock`)

## Migrating to Vitest 2.0 {#vitest-2} (part 2)

Previously `vi.fn<TArgs, TReturn>` accepted two generic types separately for arguments and return value. This is changed to directly accept a function type `vi.fn` to simplify the usage.
...
## Migrating to Vitest 1.0 (part 2)
...
## Migrating from Jest {#jest} (part 2)
