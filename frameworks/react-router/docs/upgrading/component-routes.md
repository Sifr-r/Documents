...
If you are using `` please see [Framework Adoption from RouterProvider][upgrade-router-provider] instead.

If you are using `` this is the right place.
...
- Render a `` instead of your `` component
- Note: we stopped rendering the `` component. We'll bring it back in a later step, but first we want to get the app to boot with the new entry point.
...
- Remember to _not_ try to render your existing `` component yet, we'll do that in a later step
...
To get back to rendering your app, we'll update the "catchall" route we set up earlier that matches all URLs so that your existing `` get a chance to render.
