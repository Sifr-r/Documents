...
Route actions allow server-side data mutations with automatic revalidation of all loader data on the page when called from ``, `useFetcher`, and `useSubmit`.
...
All routes links will be aggregated and rendered through the `` component, usually rendered in your app root:
...
Route meta defines [meta tags][meta-element] to be rendered in the `` component, usually placed in the `<head>`.
