    ...
    <>
      <div id="sidebar">
        ...
        <nav>
          <ul>
            <li>
              <Link to={`/contacts/1`}>Your Name
            </li>
            <li>
              <Link to={`/contacts/2`}>Your Friend
            </li>
          </ul>
        </nav>
      </div>
...
When a route has children, and you're at the parent route's path, the `` has nothing to render because no children match. You can think of [index routes][index-route] as the default child route to fill in that space.
...
export default function App() {
  return (
    <>
      <div id="sidebar">
        <h1>
          <Link to="about">React Router Contacts
        </h1>
        {/* other elements */}
      </div>
      {/* other elements */}
...
For now just return an [``][outlet-component].
...
And inside `app/root.tsx`, `App` should just return an [``][outlet-component], and all unused imports can be removed. Make sure there is no `clientLoader` in `root.tsx`.
...
This is where the "old school web" programming model shows up. [``][form-component] prevents the browser from sending the request to the server and sends it to your route's `action` function instead with [`fetch`][fetch].
...
Just like creating data, you update data with [``][form-component]. Let's make a new route module inside `app/routes/edit-contact.tsx`.
...
1. `` prevents the default browser behavior of sending a new document `POST` request to the server, but instead emulates the browser by creating a `POST` request with client side routing and [`fetch`][fetch]
...
## Synchronizing URLs to Form State (part 2)
...
👉 **Change the `` form to a fetcher form**
...
Check that out, both stars automatically update. Our new `<fetcher.Form method="post">` works almost exactly like the `` we've been using: it calls the action and then all data is revalidated automatically — even your errors will be caught the same way.
