...
## Transitions in React Router (part 2)
...
- All ``/`` navigations will be wrapped in `React.startTransition`, using the promise returned by `useNavigate`/`useSubmit` so that the Transition lasts for the duration of the navigation
...
The only APIs that are automatically wrapped in an async Transition are `` and ``. For everything else, you need to wrap the operation in `startTransition` yourself.
...
## Transitions in React Router (part 3)
