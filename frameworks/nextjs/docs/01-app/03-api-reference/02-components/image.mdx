...
When providing the `src` prop to the `` component, both the `srcset` and `src` attributes are generated automatically for the resulting `<img>`.
...
For example, when upgrading an existing website from `<img>` to ``, you may wish to maintain the same `src` attribute for SEO purposes such as image ranking or avoiding recrawl.
