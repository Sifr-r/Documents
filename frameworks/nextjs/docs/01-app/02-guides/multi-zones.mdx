...
Links to paths in a different zone should use an `a` tag instead of the Next.js [``](/docs/pages/api-reference/components/link) component. This is because Next.js will try to prefetch and soft navigate to any relative path in `` component, which will not work across zones.
