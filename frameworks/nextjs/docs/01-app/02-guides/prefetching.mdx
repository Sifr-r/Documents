...
You can extend the `` component to create your own custom prefetching strategy. For example, using the [ForesightJS](https://foresightjs.com/docs/integrations/nextjs) library which prefetches links by predicting the direction of the user's cursor.

Alternatively, you can use [`useRouter`](/docs/app/api-reference/functions/use-router) to recreate some of the native `` behavior. However, be aware this opts you into maintaining prefetching and cache invalidation.
...
For example, you may still want to have consistent usage of `` in your application, but links in your footer might not need to be prefetched when entering the viewport.
...
Next.js automatically prefetches links in the viewport when using the `` component.
...
You can disable prefetching by setting the `prefetch` prop of the `` component to `false`.
