...
>
  ...
  <ListBox
    ...
    {item => <ListBoxItem style={{height: '100%', minHeight: 0}}>{item.name}}
        ...
        <GridList
          ...
          {item => (
            <GridListItem textValue={item.title} style={{height: '100%'}}>
              ...
              {item.title}
              <Text slot="description">{item.artist}
  ...
  <GridList
    ...
    {item => (
      <GridListItem textValue={item.title}>
        ...
        {item.title}
        <Text slot="description">{item.user}
...
| `layout` | `ILayout | LayoutClass` | — | The layout object that determines the position and size of the visible elements. |
