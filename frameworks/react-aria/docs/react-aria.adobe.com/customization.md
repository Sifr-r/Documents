...
const MyCustomLabel = React.forwardRef(
  (props: LabelProps, ref: React.ForwardedRef) => {
...
This enables a `` to be placed inside a `` to display the current value.
...
const MyCheckbox = React.forwardRef((props: CheckboxProps, ref: React.ForwardedRef) => {
  // Merge the local props and ref with the ones provided via context.
...
`useContextProps(props: T & SlotProps, ref: ForwardedRef | undefined, context: Context<ContextValue<U, E>>): [T, RefObject<E | null>]`

### useSlottedContext

`useSlottedContext(context: Context<SlottedContextValue>, slot?: string | null): T | null | undefined`
