...
export interface CommandPaletteProps
  extends Omit<AriaAutocompleteProps, 'children'>, AriaMenuProps {
  isOpen: boolean;
...
function Example(props) {
  ...
  return (
    <DialogTrigger isOpen={isOpen} onOpenChange={setOpen}>
      Open Command Palette <kbd className="font-sans text-xs ml-4 px-1 rounded-sm border border-white/20 bg-white/10">⌘ J</kbd>
      {/*- begin focus -*/}
      <CommandPalette
        {...props}
    ...
    <Autocomplete filter={contains}>
      ...
      <ResizableTableContainer style={{height: 400}}>
        <Table aria-label="Stocks" selectionMode="multiple">
            ...
            <Column id="marketCap" allowsResizing>Market Cap
...
| `filter` | `((textValue: string, inputValue: string, node: Node) => boolean) | undefined` | — | An optional filter function used to determine if a option should be included in the autocomplete list. Include this if the items you are providing to your wrapped collection aren't filtered by default. |
