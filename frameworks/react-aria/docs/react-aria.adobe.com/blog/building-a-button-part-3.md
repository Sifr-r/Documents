# building-a-button-part-3

## Introduction

This is the last post in our three part series on building a button component. In the [first post](building-a-button-part-1.md), we covered how React Spectrum and React Aria implement adaptive press events across mouse, touch, keyboard, and screen readers. In the [second post](building-a-button-part-2.md), we covered hover interactions. Today, we'll cover keyboard focus behavior.
...
<video
  src={focusRingVideoUrl}
  loop
  autoPlay
  muted
  playsInline
  style={{maxWidth: 'min(100%, 640px)', display: 'block', margin: '20px auto'}}
/>
...
<img
  src={keyboardSettingsImageUrl}
  alt="Screenshot of macOS keyboard system preferences"
  style={{maxWidth: 'min(100%, 780px)', display: 'block', margin: '20px auto'}}
/>
...
This focus normalization behavior is implemented by the [usePress](../usePress.md) hook in React Aria, which is used by [useButton](../Button/useButton.md), [useCheckbox](../Checkbox/useCheckbox.md), [useRadio](../RadioGroup/useRadioGroup.md), and many other hooks. If you're implementing your own components, I'd highly recommend checking them out to ensure the focus behavior is consistent across browsers.
