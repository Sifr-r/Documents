    ...
    <ToastRegion queue={queue}>
      {({toast}) => (
        <MyToast toast={toast} style={{viewTransitionName: toast.key} as CSSProperties}>
            ...
            <Text slot="title">{toast.content.title}
            {toast.content.description && (
              <Text slot="description">{toast.content.description}
            )}
...
<InlineAlert variant="notice">
  Accessibility
  We recommend that the close button should be rendered as a sibling of `` rather than inside it, so that screen readers announce the toast content without the close button first.
