...
1. The <CodeStep step={2}>optimistic state, initially set to the <CodeStep step={1}>value provided.
2. The <CodeStep step={3}>set function that lets you temporarily change the state during an [Action](reference/react/useTransition#functions-called-in-starttransition-are-called-actions).
   * If a <CodeStep step={4}>reducer is provided, it will run before returning the optimistic state.

To use the <CodeStep step={2}>optimistic state, call the `set` function inside an Action.
