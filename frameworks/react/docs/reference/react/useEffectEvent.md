...
`useEffectEvent` accepts an `event callback` and returns an <CodeStep step={1}>Effect Event. The Effect Event is a function that can be called inside of Effects without re-connecting the Effect:
...
Since `onConnected` is an <CodeStep step={1}>Effect Event, `muted` and `onConnect` are not in the Effect dependencies.
