...
You will receive a <CodeStep step={1}>ref as the second argument after props. Pass it to the DOM node that you want to expose:
...
This lets the parent `Form` component access the <CodeStep step={2}>`<input>` DOM node exposed by `MyInput`:
