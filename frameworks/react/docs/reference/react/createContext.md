...
`createContext` returns a <CodeStep step={1}>context object. Components can read context by passing it to [`useContext()`](/reference/react/useContext):
...
By default, the values they receive will be the <CodeStep step={3}>default values you have specified when creating the contexts. However, by itself this isn't useful because the default values never change.
