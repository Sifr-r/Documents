...
console.log(clonedElement); // <Row title="Cabbage" isHighlighted={true}>Goodbye
```
...
To override the props of some <CodeStep step={1}>React element, pass it to `cloneElement` with the <CodeStep step={2}>props you want to override:
...
Here, the resulting <CodeStep step={3}>cloned element will be `<Row title="Cabbage" isHighlighted={true} />`.
