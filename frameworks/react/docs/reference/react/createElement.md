...
* When you use JSX, **you must start a tag with a capital letter to render your own custom component.** In other words, `` is equivalent to `createElement(Something)`, but `<something />` (lowercase) is equivalent to `createElement('something')` (note it's a string, so it will be treated as a built-in HTML tag).
...
To create an element without JSX, call `createElement` with some <CodeStep step={1}>type, <CodeStep step={2}>props, and <CodeStep step={3}>children:
...
The <CodeStep step={3}>children are optional, and you can pass as many as you need (the example above has three children). This code will display a `<h1>` header with a greeting. For comparison, here is the same example rewritten with JSX:
...
To render your own React component, pass a function like `Greeting` as the <CodeStep step={1}>type instead of a string like `'h1'`:
