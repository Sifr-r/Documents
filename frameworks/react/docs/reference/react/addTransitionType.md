...
- Transition Types are reset after each commit. This means a `` fallback will associate the types after a `startTransition`, but revealing the content does not.
...
When you call <CodeStep step={1}>addTransitionType inside the scope of <CodeStep step={2}>startTransition, React will associate <CodeStep step={3}>submit-click as one of the causes for the Transition.
