...
Along with the <CodeStep step={1}>root component, you need to provide a list of <CodeStep step={2}>bootstrap `<script>` paths. Your root component should return **the entire document including the root `<html>` tag.**
...
React will inject the [doctype](https://developer.mozilla.org/en-US/docs/Glossary/Doctype) and your <CodeStep step={2}>bootstrap `<script>` tags into the resulting HTML stream:
...
Imagine that loading data for `` takes some time. Ideally, you'd want to show the rest of the profile page content to the user without waiting for the posts. To do this, [wrap `Posts` in a `` boundary:](/reference/react/Suspense#displaying-a-fallback-while-content-is-loading)
...
You can further [nest `` boundaries](/reference/react/Suspense#revealing-nested-content-as-it-loads) to create a more granular loading sequence:
...
The part of your app outside of any `` boundaries is called *the shell:*
