...
In lieu of using hidden form fields to provide data to the `<form>`'s action, you can call the <CodeStep step={1}>`bind` method to supply it with extra arguments. This will bind a new argument (<CodeStep step={2}>`productId`) to the function in addition to the <CodeStep step={3}>`formData` that is passed as an argument to the function.
