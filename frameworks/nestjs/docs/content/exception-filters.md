...
> info **Hint** All exception filters should implement the generic `ExceptionFilter` interface. This requires you to provide the `catch(exception: T, host: ArgumentsHost)` method with its indicated signature. `T` indicates the type of the exception.
