...
export class KillDragonHandler implements ICommandHandler {
  ...
  async execute(command: KillDragonCommand) {
    ...
    // "ICommandHandler" forces you to return a value that matches the command's return type
    return {
...
Note that `ICommandHandler` forces you to return a value that matches the command's return type. In this case, the return type is an object with an `actionId` property. This only applies to commands that inherit from the `Command` class. Otherwise, you can return whatever you want.
...
Similar to the `Command` class, the `Query` class is a simple utility class exported from the `@nestjs/cqrs` package that lets you define the query's return type. In this case, the return type is a `Hero` object. Now, whenever the `GetHeroQuery` query is dispatched, the `QueryBus#execute()` method return-type will be inferred as `Promise`.
...
export class GetHeroHandler implements IQueryHandler {
  constructor(private repository: HeroesRepository) {}
...
export class HeroesGameSagas {
  @Saga()
  dragonKilled = (events$: Observable<any>): Observable => {
    return events$.pipe(
