...
With this in place, we can define a `PaginatedDto` response, as follows:
...
In this example, we specify that the response will have allOf `PaginatedDto` and the `results` property will be of type `Array`.
...
For client generation tools, this approach poses an ambiguity in how the `PaginatedResponse` is being generated for the client. The following snippet is an example of a client generator result for the above `GET /` endpoint.
