...
> info **Hint** The `NestInterceptor<T, R>` is a generic interface in which `T` indicates the type of an `Observable` (supporting the response stream), and `R` is the type of the value wrapped by `Observable`.
...
@Injectable()
export class TransformInterceptor implements NestInterceptor<T, Response> {
