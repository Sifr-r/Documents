...
function customMount(component: string | Type, config?: MountConfig) {
  if (!config) {
    config = { declarations }
  } else {
    config.declarations = [...(config?.declarations || []), ...declarations]
...
Cypress.Commands.add(
  'mount',
  (component: Type | string, config?: MountConfig) => {
    return mount(component, {
      ...config,
      autoSpyOutputs: true,
    })
