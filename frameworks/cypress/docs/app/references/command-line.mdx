...
Set secret [environment variables](/app/guides/environment-variables) for use in your tests via [`cy.env()`](/api/commands/env). See the [Environment Variables & Secrets](/app/guides/environment-variables) guide for more details.
...
#### `cypress run --expose <expose>` {#cypress-run-expose-lt-expose-gt}

Set public configuration values for use in your tests via [`Cypress.expose()`](/api/cypress-api/expose). See the [Environment Variables & Secrets](/app/guides/environment-variables) guide for more details.
...
#### `cypress run --pass-with-no-tests` {#cypress-run-pass-with-no-tests}
...
#### `cypress open --expose <expose>` {#cypress-open-expose-lt-expose-gt}
