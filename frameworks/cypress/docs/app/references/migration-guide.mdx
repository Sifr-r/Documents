...
Starting with Cypress 15.10.0, `Cypress.env()` has been deprecated and will be removed in a future major version. This guide helps you migrate to [`cy.env()`](/api/commands/env) (for sensitive values) or [`Cypress.expose()`](/api/cypress-api/expose) (for public/non-sensitive values), depending on your use case.
...
1. **Cross-origin risks**: When using [`cy.origin()`](/api/commands/origin) for cross-origin testing, all environment variables are automatically passed to the cross-origin context, potentially exposing sensitive data to untrusted origins.
1. **Application code access**: Since environment variables are available in the browser context when tests run, your application code or third-party scripts could potentially access them if it checks for `window.Cypress` or similar patterns.
...
<Badge type="danger">Before: Cypress.env()
...
<Badge type="success">After: cy.env()

## Migrating away from Cypress.env() (part 2)
...
<Badge type="success">After: Cypress.expose()
...
<Badge type="danger">Before: Cypress.env() CLI flags

```shell
cypress run --env FEATURE_FLAG=true,API_VERSION=v2,PUBLIC_API_URL=https://api.example.com
```

<Badge type="success">After: Cypress.expose() CLI flags
...
## Migrating away from Cypress.env() (part 3)
...
## Migrating away from Cypress.env() (part 4)
...
1. ✅ Replace each `Cypress.env()` call with either `cy.env()` or `Cypress.expose()`
1. ✅ Update code to handle the asynchronous nature of `cy.env()`
1. ✅ Migrate plugins that use `Cypress.env()`
1. ✅ Set `allowCypressEnv: false` in your Cypress configuration
1. ✅ Verify that no errors are thrown
...
<Badge type="danger">Before

```javascript
cy.exec('rake db:seed').its('code').should('eq', 0)
```

<Badge type="success">After
...
## Migrating to Cypress 15.0 (part 2)
...
## Migrating to Cypress 15.0 (part 3)
...
## Migrating to Cypress 14.0 (part 2)
...
## Migrating to Cypress 14.0 (part 3)
...
## Migrating to Cypress 14.0 (part 4)
...
## Migrating to Cypress 14.0 (part 5)
...
## Migrating to Cypress 13.0 (part 2)
...
<Badge type="danger">Before Multiple small tests against different
origins
...
## Migrating to Cypress 12.0 (part 2)
...
<Badge type="success">After One big test using `cy.origin()`
...
<Badge type="danger">For example the following tests are not independent
nor deterministic:
...
## Migrating to Cypress 12.0 (part 3)
...
## Migrating to Cypress 12.0 (part 4)
...
## Migrating to Cypress 12.0 (part 5)
...
## Migrating to Cypress 12.0 (part 6)
...
## Migrating to Cypress 11.0 (part 2)
...
## Migrating to Cypress 11.0 (part 3)
...
## Migrating to Cypress 10.0 (part 2)
...
## Migrating to Cypress 10.0 (part 3)
...
## Migrating to Cypress 10.0 (part 4)
...
<Badge type="danger">Before `cypress/plugins/index.js`
...
## Migrating to Cypress 10.0 (part 5)
...
## Migrating to Cypress 10.0 (part 6)
...
## Migrating to Cypress 10.0 (part 7)
