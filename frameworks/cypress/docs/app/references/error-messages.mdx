# Common Error Messages in Cypress
...
<DocsImage src="/img/app/references/no-tests-found.png" alt="No tests found" />
...
[`0.18.0`](/app/references/changelog#0-18-0) and was replaced by module
support and the
...
[Examples](/app/references/recipes) or check out our
[Support channels](/app/references/troubleshooting#Support-channels) for more
...
<DocsImage
  ...
  alt="cy.method() failed because element is animating"
/>

## Command Errors (part 3)
...
modifying the [Cypress configuration](/app/references/configuration).
...
<DocsImage
  ...
  alt="The test has finished but Cypress still has commands"
/>

:::caution

<strong>Flaky tests below!</strong>
...
See our [Cross Origin Testing](/app/guides/cross-origin-testing#Limitations) documentation.
...
In version [0.20.0](/app/references/changelog), we removed the commands for
...
[Continuous Integration](/app/continuous-integration/overview). This
means that you did not pass a specific record key to:
[cypress run --record](/app/references/command-line#cypress-run).
...
[--group](/app/references/command-line#cypress-run-group-lt-name-gt) or
[--parallel](/app/references/command-line#cypress-run-parallel) flag.
...
Check out our
[guide on parallelizing runs](/cloud/features/smart-orchestration/parallelization)
and when to use the
[--ci-build-id](/app/references/command-line#cypress-run-ci-build-id-lt-id-gt)
option.

### <Icon name="exclamation-triangle" color="red" /> The `--ci-build-id`, `--group`, `--tag`, `--parallel`, or `--auto-cancel-after-failures` flags can only be used when recording

You passed the `--ci-build-id`,
[--group](/app/references/command-line#cypress-run-group-lt-name-gt),
[--tag](/app/references/command-line#cypress-run-tag-lt-tag-gt),
[--parallel](/app/references/command-line#cypress-run-parallel), or
...
You passed the
[--group](/app/references/command-line#cypress-run-group-lt-name-gt) or
[--parallel](/app/references/command-line#cypress-run-parallel) flag but we could
...
[CI providers](/app/continuous-integration/overview#CI-Examples). Please
review the
[natively recognized environment variables](/cloud/features/smart-orchestration/parallelization#CI-Build-ID-environment-variables-by-provider)
...
You passed the
[--group](/app/references/command-line#cypress-run-group-lt-name-gt) flag, but
this group name has already been used for this run.

If you are trying to parallelize this run, then also pass the
[--parallel](/app/references/command-line#cypress-run-parallel) flag, else pass a
different group name.

Please review
[grouping test runs](/cloud/features/smart-orchestration/parallelization#Grouping-test-runs)
documentation to learn more.

### <Icon name="exclamation-triangle" color="red" /> Cannot parallelize tests across environments

You passed the [--parallel](/app/references/command-line#cypress-run-parallel)
...
You cannot use the
[--parallel](/app/references/command-line#cypress-run-parallel) flag with this
group.
...
You must use the [--parallel](/app/references/command-line#cypress-run-parallel)
...
[--parallel](/app/references/command-line#cypress-run-parallel) flag to a run
...
[parallelization doc](/cloud/features/smart-orchestration/parallelization#CI-Build-ID-environment-variables-by-provider).
...
When a run finishes all of its groups, it waits for a configurable set of time, [a run completion delay](/cloud/features/smart-orchestration/parallelization#Run-completion-delay),
before completing. All `cypress run` calls with any new groups must be executed during that time period.

#### Troubleshooting

- If you are passing [`--ci-build-id`](/app/references/command-line#cypress-run-ci-build-id-lt-id-gt), make sure it is generating a unique value for the run. If it is not unique and matches a previous run, you may see this error.
- If you are running `cypress run` calls in parallel and they are not completing within the default 60 second run completion delay, you can increase this delay. [See instructions](/cloud/account-management/projects#Run-completion-delay).
...
### <Icon name="exclamation-triangle" color="red" /> The Cypress App could not be unzipped. This is most likely because the maximum path length is being exceeded on your system.
...
### <Icon name="exclamation-triangle" color="red" /> error: unknown option: --auto-cancel-after-failures
...
### <Icon name="exclamation-triangle" color="red" /> --auto-cancel-after-failures passed without a Business or Enterprise Cloud account
...
### <Icon name="exclamation-triangle" color="red" /> You passed the --auto-cancel-after-failures flag for a run that is already in progress
...
For a more thorough explanation of Cypress's Cross Origin Testing,
[please read our dedicated guide to it](/app/guides/cross-origin-testing).
...
1. Don't click `<a>` links in your tests that navigate outside of your
   ...
   [in our "Tab Handling and Links" example recipe](/app/references/recipes#Testing-the-DOM).
...
[Cypress configuration](/app/references/configuration) when running in Chrome
family browsers (this setting will not work in other browsers). Before doing so
you should really understand and
[read about the reasoning here](/app/guides/cross-origin-testing).
...
2. Use the built-in Electron browser for tests, since it is not affected by
   these policies.
   [See the guide to launching browsers for more information.](/app/references/launching-browsers#Electron-Browser)
