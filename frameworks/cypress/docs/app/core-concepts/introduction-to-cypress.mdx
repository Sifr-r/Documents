...
- How Cypress uses command chains and the asynchronous nature of Cypress commands
- How to interact with elements and assert about elements
...
[customizable timeouts](/app/references/configuration#Timeouts), Cypress
...
timeout periods
([the default timeout is 4 seconds](/app/references/configuration#Timeouts)).
...
[some guarantees](/app/core-concepts/interacting-with-elements) about what
...
called [Aliases](/app/core-concepts/variables-and-aliases). It looks
something like this:
...
This lets us reuse our queries for more readable tests, and it automatically
...
it('hides the thing when it is clicked', () => {
  -{cy.visit('/my/resource/path')::cy.mount()}- // Nothing happens yet
...
```js
it('hides the thing when it is clicked', () => {
  -{cy.visit('/my/resource/path')::cy.mount()}- // 1.
...
While the API may look similar to Promises, with its `then()` syntax, Cypress
...
**[retry-ability](/app/core-concepts/retry-ability)**. Without
**[retry-ability](/app/core-concepts/retry-ability)**, assertions would
...
[conditional testing](/app/guides/conditional-testing) here.
