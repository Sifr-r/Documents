...
fails, check out our guide on [Test Retries](/app/guides/test-retries).
...
The [Command Log](/app/core-concepts/open-mode#Command-Log) shows all
...
## Commands, Queries and Assertions (part 2)
...
indicator, meaning Cypress is still waiting for the assertion to pass.
...
function with 2 [`expect`](/app/references/assertions#Chai)
...
_waiting_ and assertions. For example, as described in the "Assertions" section
of [.click()](/api/commands/click), the `click()` action command waits to click
...
[Action commands](/app/core-concepts/interacting-with-elements) - such as
`.click()` - automatically wait until multiple built-in assertions like these
pass, and then it will attempt the action once.

## Timeouts

By default each command that retries does so for up to 4 seconds - the
[`defaultCommandTimeout`](/app/references/configuration#Timeouts) setting.

### Increase time to retry

You can change the default timeout for _all commands_. See
[Configuration: Overriding Options](/app/references/configuration#Overriding-Options)
...
examples, read the
[Timeouts](/app/core-concepts/introduction-to-cypress#Timeouts) section in
...
## Only queries are retried (part 2)
...
## Only queries are retried (part 3)
