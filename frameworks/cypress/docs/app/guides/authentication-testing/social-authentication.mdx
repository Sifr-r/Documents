# Social Authentication: Cypress Guide
...
{
  env: {
    MICROSOFT_USERNAME: process.env.MICROSOFT_USERNAME,
    MICROSOFT_PASSWORD: process.env.MICROSOFT_PASSWORD,
    MICROSOFT_NAME: process.env.MICROSOFT_NAME,
    GOOGLE_USERNAME: process.env.GOOGLE_USERNAME,
    GOOGLE_PASSWORD: process.env.GOOGLE_PASSWORD,
    GOOGLE_NAME: process.env.GOOGLE_NAME,
    FACEBOOK_USERNAME: process.env.FACEBOOK_USERNAME,
    FACEBOOK_PASSWORD: process.env.FACEBOOK_PASSWORD,
    FACEBOOK_NAME: process.env.FACEBOOK_NAME,
  },
}
```
...
## Setting Auth0 app credentials in Cypress (part 2)
    ...
    switch (SOCIAL_PROVIDER) {
      case 'microsoft':
        cy.env([
          ...
          'MICROSOFT_PASSWORD',
          'MICROSOFT_NAME',
        ]).then(({ microsoftUsername, microsoftPassword, microsoftName }) => {
          logIntoMicrosoft(microsoftUsername, microsoftPassword, microsoftName)
        })
        break
      case 'google':
        cy.env(['GOOGLE_USERNAME', 'GOOGLE_PASSWORD', 'GOOGLE_NAME']).then(
          ({ googleUsername, googlePassword, googleName }) => {
            logIntoGoogle(googleUsername, googlePassword, googleName)
          }
        )
        break
      case 'facebook':
        cy.env([
          ...
          'FACEBOOK_PASSWORD',
          'FACEBOOK_NAME',
        ]).then(({ facebookUsername, facebookPassword, facebookName }) => {
          logIntoFacebook(facebookUsername, facebookPassword, facebookName)
        })
        break
      default:
...
## Setting Auth0 app credentials in Cypress (part 3)
...
## Setting Auth0 app credentials in Cypress (part 4)
...
cy.session(
        ...
        switch (SOCIAL_PROVIDER) {
          case 'microsoft':
            cy.env([
              ...
              ({ microsoftUsername, microsoftPassword, microsoftName }) => {
                logIntoMicrosoft(
                  microsoftUsername,
                  microsoftPassword,
                  microsoftName
      ...
      },
      {
        validate: () => {
          ...
          switch (SOCIAL_PROVIDER) {
            case 'microsoft':
              cy.env(['MICROSOFT_NAME']).then(({ microsoftName }) => {
                cy.get('h6.dropdown-header').should('contain', microsoftName)
              })
              break
            case 'google':
              cy.env(['GOOGLE_NAME']).then(({ googleName }) => {
                cy.get('h6.dropdown-header').should('contain', googleName)
              })
              break
            case 'facebook':
              cy.env(['FACEBOOK_NAME']).then(({ facebookName }) => {
                cy.get('h6.dropdown-header').should('contain', facebookName)
              })
              break
            default:
              throw new Error('no social provider configured!')
