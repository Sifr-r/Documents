...
- How Cypress executes tests: command chaining, retry-ability, and what
...
- Parallelization, Smart Orchestration, Test Replay, and accessibility
  testing in Cypress Cloud
...
<Badge type="danger">Before: Playwright
...
<Badge type="success">After: Cypress
...
## Configuration migration (part 2)
...
| `use.proxy`      | OS environment variables                 | See [Proxy Configuration](/app/references/proxy-configuration)                              |
...
## CLI/Command line migration (part 2)

| Playwright                         | Cypress                                                                                                                          |
| ---------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `--config=playwright.config.ts`    | `--config-file cypress.config.ts`                                                                                                |
| `npx playwright test todo.spec.ts` | `cypress run --spec "todo.cy.ts"`                                                                                                |
| `--fail-on-flaky-tests`            | `detect-flake-but-always-fail` in config with [Experimental Test Retries](/app/references/experiments#Experimental-Test-Retries) |
...
| `--last-failed`                    | [Spec Prioritization](/cloud/features/smart-orchestration/spec-prioritization)                                                   |
...
| `--workers=4`                      | `--parallel`                                                                                                                     |
| `--trace on`                       | `--record` automatically records [Test Replay](/cloud/features/test-replay)                                                      |

### Passing environment variables

## CLI/Command line migration (part 3)
...
See [cy.env()](/api/commands/env) and
[Cypress.expose()](/api/cypress-api/expose) for the full reference.
...
browser version your tests run against.
...
commands. Cypress Studio is a replacement for Playwright's `playwright codegen`. With a [Cypress Cloud](/cloud/get-started/introduction) account, you also get **Studio AI**, AI-powered assertion recommendations as you record.
...
```tsx title="button.test.ts"
import { test, expect } from '@playwright/experimental-ct-react'
import { Button } from './Button'

test('renders', async ({ mount }) => {
  const component = await mount(Save)
...
<Badge type="success">After: Cypress Cloud
