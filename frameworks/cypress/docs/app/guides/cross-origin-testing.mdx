# Cross Origin Testing: Cypress Guide
...
same [origin](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Origin) for the
...
[visit](/api/commands/visit) two different origins in the same test and continue
to interact with the page without the use of the
...
[visit](/api/commands/visit) two or more origins in **different** tests without
needing [`cy.origin()`](/api/commands/origin). :::
