# Frequently Asked Questions | Cypress Documentation
...
## General Cypress Questions (part 2)
...
## General Cypress Questions (part 3)
...
- Are able to run both locally and on a
  [continuous integration](/app/continuous-integration/overview) server.

## General Cypress Questions (part 4)
...
## General Cypress Questions (part 5)
...
## "How-to" Cypress Questions (part 2)
...
## "How-to" Cypress Questions (part 3)
...
## "How-to" Cypress Questions (part 4)
...
## "How-to" Cypress Questions (part 5)
...
## "How-to" Cypress Questions (part 6)
...
3. You can use a 3rd party email service that provides temporary email addresses
   ...
   <a href="/app/plugins/plugins-list#email">Cypress plugin</a> to access
   emails.
...
You can use [`cy.request()`](/api/commands/request) or [`cy.task()`](/api/commands/task) to talk
...
There are three common approaches, depending on your use case:

**Option 1: Re-set a known cookie value before each test**

If you know the cookie value ahead of time (for example, a cookie consent banner
or a feature flag cookie), use [`cy.setCookie()`](/api/commands/setcookie) in a
`beforeEach` hook so the cookie is present at the start of every test:
...
## "How-to" Cypress Questions (part 7)

**Option 2: Use `cy.session()` to cache and restore server-set cookies**

If your cookie is generated by the server after login (e.g. a session or auth
token cookie), use [`cy.session()`](/api/commands/session) to capture and replay
the full browser context — cookies, localStorage, and sessionStorage — for every
subsequent test without repeating the login flow:
...
beforeEach(() => {
  login('myuser', 'password123')
  cy.visit('/dashboard')
})
```

**Option 3: Disable test isolation for a suite**

You can disable [`testIsolation`](/app/references/configuration#e2e) at the
`describe` level to prevent Cypress from clearing browser state between tests in
that suite. Use a `before` hook (not `beforeEach`) to set up state once:

```javascript
describe('suite with shared cookies', { testIsolation: false }, () => {
  before(() => {
    cy.setCookie('cookieConsent', 'accepted')
    cy.visit('/home')
  })
  ...
  it('test one', () => {
    // cookieConsent cookie is present
  })
...
Disabling test isolation means tests may affect each other's state. Ensure each
test can still pass independently before relying on this approach.
...
## "How-to" Cypress Questions (part 8)
...
## "How-to" Cypress Questions (part 9)
...
## "How-to" Cypress Questions (part 10)
...
## "How-to" Cypress Questions (part 11)
...
## Continuous Integration (CI/CD) (part 2)
...
## Distinct Cypress Use Cases (part 2)
...
## Integrations with Other Tools/Frameworks/Libraries (part 2)
