# Detect changes | Cypress Accessibility Documentation
...
- The [Results API](/accessibility/guides/results-api) to fail builds or trigger alerts when results don't meet your standards
...
Cypress Accessibility's [Results API](/accessibility/guides/results-api) provides rich data related to the outcome of any accessibility report for a Cypress run. It supports adding a step in CI to block pull requests or trigger other side effects. To use the Results API, you'll need to [add a step in your CI process](/accessibility/guides/results-api#Installation). This comes with a helper function which will automatically match your CI build with the related run in Cypress Cloud.
...
Analytics live under the [Cypress Accessibility](/cloud/features/analytics/enterprise-reporting#Cypress-Accessibility) tab of [Enterprise Reporting](/cloud/features/analytics/enterprise-reporting) in Cypress Cloud, alongside other org-wide metrics on the same page.

The same aggregates you see in the UI can be pulled programmatically: every chart is backed by data you can [download from Cloud](/cloud/features/analytics/enterprise-reporting#Download-data-from-Cloud) or request through the [Data Extract API](/cloud/integrations/data-extract-api) (see [Retrieve your test data through API](/cloud/features/analytics/enterprise-reporting#Retrieve-your-test-data-through-API) on the Enterprise Reporting page).

That API access supports many possible uses, including:

- Integrating Cypress Accessibility data with other quality metrics in your internal dashboards
- Creating one-off reports about improvements or successes (for example showing the benefits over time that come from the adoption of your design system)
- AI workflows: react to trends to find specific runs and drill into views, rules, and elements using [Cloud MCP](/cloud/integrations/cloud-mcp) tools and [Work with AI agents](/accessibility/work-with-ai-agents).

### Example accessibility charts

The following screenshots illustrate typical cards on the Cypress Accessibility tab and how you might interpret them.
...
This layout gives a north-star percentage for the filtered scope, a quick comparison of which projects score highest, and trends in topline scores over time. Use **Start date**, **End date**, **Team**, **Projects**, and **Branch** filters to zoom from org-wide trends to a single branch.
...
The **severity breakdown** summarizes failed rule categories across the entire set of runs included in your filters.
...
Tracking the number of failed elements per rule over time helps separate out permanent changes from brief ones.

If something is merged with new accessibility failures, but the failures are quickly fixed, those are unlikely to appear here - they are blips with little impact.

## Analytics (part 2)

On the other hand, when a rule has new failures introduced that are not quickly repaired, that will be clear in this chart - as the `region` rules above shows.

Likewise, when a rule's failures are fixed, a downward line here means the fix "stuck" and was picked up consistently in later runs.
...
This view emphasizes how many distinct rules fail per run on average, stacked by impact. It complements severity counts by showing whether breadth of rule failures is growing or shrinking week over the time period in question.
...
**Element-level violations by impact** highlight volume and volatility: spikes often indicate widespread markup or content issues.

#### Enterprise Reporting

For more context on the Enterprise Reporting product and other tabs, see [Enterprise Reporting](/cloud/features/analytics/enterprise-reporting) and the [Analytics](/cloud/features/analytics/overview) overview. For endpoint details and export formats, use the [Data Extract API](/cloud/integrations/data-extract-api) documentation.

### Analytics use cases

#### Track progress and spot trends

This level of reporting is ideal for documenting progress and spotting unexpected trends. When you see a project's score dropping over the long term, you can explore further with Branch Review to understand what changes are driving the score.

You can compare reports before and after any shifts in scores to determine which accessibility issues were introduced.

#### Pull high level metrics for AI analysis

You can dive deeper with [Cloud MCP](/cloud/integrations/cloud-mcp) for your AI agents to look into issue ownerships remediation planning, and configuration.
