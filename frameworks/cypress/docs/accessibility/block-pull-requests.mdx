# Block pull requests and set policies | Cypress Accessibility Documentation
...
The [Cypress Accessibility Results API](/accessibility/guides/results-api) allows you to access accessibility results post-test run, enabling workflows like blocking pull requests or triggering alerts based on specific accessibility criteria. This involves adding a dedicated accessibility verification step to your CI pipeline. With a Cypress helper function, you can automatically fetch the report for the relevant test run within the CI build context. Below is an example of how to block a pull request in GitHub Actions if accessibility violations occur:
...
The [Results API Documentation](/accessibility/guides/results-api) provides detailed guidance on the API's capabilities. Here's a simplified example demonstrating how to enforce a "no new failing accessibility rules" policy:
...
## Comparing against a baseline {#comparing-against-a-baseline} (part 2)
...
## Comparing against a baseline {#comparing-against-a-baseline} (part 3)
