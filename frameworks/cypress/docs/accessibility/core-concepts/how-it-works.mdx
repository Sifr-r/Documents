# Cypress Accessibility | How it works
...
1. **Displaying results**: Results are displayed in Cypress Cloud in the run details area as well as in [Branch Review](/accessibility/core-concepts/compare-reports) for comparing runs.
1. **Retreiving results**: Results are accessible via the [Results API](/accessibility/guides/results-api) for optional CI pipeline actions (e.g., failing builds based on unmet standards) and via Cloud MCP for agentic triage.
