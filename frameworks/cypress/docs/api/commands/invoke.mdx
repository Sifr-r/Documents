...
| `timeout` | [`defaultCommandTimeout`](/app/references/configuration#Timeouts) | Time to wait for `.invoke()` to resolve before [timing out](#Timeouts)              |
...
[Check out our example recipe where we use `cy.invoke('text')` to test against HTML content in 'Bootstrapping your App'](/app/references/recipes#Server-Communication)
...
[Check out our example recipe where we use `cy.invoke('show')` and `cy.invoke('trigger')` to click an element that is only visible on hover](/app/references/recipes#Testing-the-DOM)
...
[Check out our example recipe where we use `cy.invoke('removeAttr', 'target')` to test clicking on a link without opening in a new tab](/app/references/recipes#Testing-the-DOM)
...
| Version                                    | Changes                                                       |
| ------------------------------------------ | ------------------------------------------------------------- |
| [12.0.0](/app/references/changelog#12-0-0) | `.invoke()` no longer supports promises or async functions    |
| [3.8.0](/app/references/changelog#3-8-0)   | Added support for `options` argument                          |
| [3.7.0](/app/references/changelog#3-7-0)   | Added support for arguments of type Number for `functionName` |
