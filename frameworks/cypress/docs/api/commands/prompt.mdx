...
- [Setup](#Setup) — Configure authentication
...
`cy.prompt` self-heals when a **selector** becomes invalid between runs. Self-healing works
best when it catches occasional, unexpected changes — **not when it fires on every run**.

If you use `cy.prompt` to target an element whose content changes frequently (like
...
To avoid self-healing on every run, you can:

## Choose your workflow (part 2)

- Target an element by its content. In this case, you should consider using [text-based targeting](#Targeting-elements-by-text-content).
- Use placeholders to store a dynamic value of the element you are targeting. In this case, you could consider using [placeholders](#Placeholders).
- Update the ElementSelector API to prioritize a different attribute or selector that is more stable. In this case, you could consider using [ElementSelector API](/api/cypress-api/element-selector-api).

The right targeting strategy is the one that aligns with what is actually stable in
your application.
...
When test content is not specified,Cypress resolves the element by its structural role in the DOM. This resolves to `cy.get(selector)` and is better suited for elements where the selector is stable and the content may change
— for example, a submit button whose label varies by locale.
...
You can verify that elements do not exist to generate `not.exist` assertions, but **only when you provide an exact CSS selector or when you use [text-based targeting](#Targeting-elements-by-text-content)**. Natural language descriptions for non-existence are not currently supported. Additionally, ensure that your intention is to verify that the element does not exist in the DOM, not that it is not visible.
...
The placeholder option provides a way to use dynamic and sensitive values while maintaining cache efficiency.

### The caching problem

`cy.prompt` works by caching your prompt steps so it doesn't need to reach out to the AI every time. This makes tests run faster, and caching is honored across machines and environments, including CI. This means that once a prompt is cached, all team members and CI runs benefit from the cached code without additional AI calls.

However, there's a challenge: **any change to your prompt steps invalidates the cache**, forcing a new AI call.
...
- **Click the "Code" button** - This button appears next to each `cy.prompt` command in the Command Log (event for failing tests)
...
| Assertions       | 'not.exist' assertions from natural language are not supported. However, 'not.exist' assertions work when you provide an exact CSS selector (e.g., `'verify #error-message does not exist'`) or when you use [text-based targeting](#Targeting-elements-by-text-content). [See issue](https://github.com/cypress-io/cypress/issues/33274) |
