...
globally in [configuration](/app/references/configuration).
...
| `timeout`                  | [`responseTimeout`](/app/references/configuration#Timeouts) | Time to wait for `cy.request()` to resolve before [timing out](#Timeouts)                                                                                                                                |
...
[Check out our example recipe using `cy.request()` for HTML web forms](/app/references/recipes#Logging-In)
