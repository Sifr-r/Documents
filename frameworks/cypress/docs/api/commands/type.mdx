...
| `timeout`                    | [`defaultCommandTimeout`](/app/references/configuration#Timeouts)           | Time to wait for `.type()` to resolve before [timing out](#Timeouts)                                                                            |
...
[Check out our example recipe of logging in by typing username and password in HTML web forms](/app/references/recipes#Logging-In)
...
it('navigates form using keyboard only', () => {
  ...
  cy.press(Cypress.Keyboard.Keys.TAB)
  cy.get('input[name="email"]').should('have.focus').type('marcus@example.com')
...
- `.type()` will automatically wait for the element to reach an
  [actionable state](/app/core-concepts/interacting-with-elements).
- `.type()` will automatically [retry](/app/core-concepts/retry-ability)
  until all chained assertions have passed.

- `.type()` can time out waiting for the element to reach an
  [actionable state](/app/core-concepts/interacting-with-elements).
...
| [5.5.0](/app/references/changelog#5-5-0)     | Support `beforeinput` event                                                                                                            |
| [3.4.1](/app/references/changelog#3-4-1)     | Added `parseSpecialCharSequences` option                                                                                               |
| [3.3.0](/app/references/changelog#3-3-0)     | Added `{insert}`, `{pageup}` and `{pagedown}` character sequences                                                                      |
| [3.2.0](/app/references/changelog#3-2-0)     | Added `{home}` and `{end}` character sequences                                                                                         |
| [0.20.0](/app/references/changelog#0-20-0)   | Supports for typing in inputs of type `date`, `time`, `month`, and `week`                                                              |
| [0.17.1](/app/references/changelog#0-17-1)   | Added `ctrl`, `cmd`, `shift`, and `alt` keyboard modifiers                                                                             |
| [0.16.3](/app/references/changelog#0-16-3)   | Supports for typing in elements with `tabindex` attribute                                                                              |
| [0.16.2](/app/references/changelog#0-16-2)   | Added `{downarrow}` and `{uparrow}` character sequences                                                                                |
| [0.8.0](/app/references/changelog#0-8-0)     | Outputs Key Events Table to console on click                                                                                           |
| [0.8.0](/app/references/changelog#0-8-0)     | Added `{selectall}`, `{del}`, `{backspace}`, `{esc}`, `{{}`, `{enter}`, `{leftarrow}`, `{rightarrow}` character sequences              |
| [0.8.0](/app/references/changelog#0-8-0)     | Added small delay (10ms) between each keystroke during `cy.type()`                                                                     |
