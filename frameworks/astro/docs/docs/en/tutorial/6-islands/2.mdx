...
2. Import and add the `` component to `Header.astro` so that it will be displayed on all pages. Wrap both `` and `` inside a `<div>` to group them together for styling, and add the `<style>` tag as shown below for some basic styles to improve the layout.
