...
5. Add a `<slot />` element to `src/layouts/BaseLayout.astro` just above the footer component, then check the browser preview of your Home page and notice what really _did_ change this time!
 ...
 The `<slot />` allows you to inject (or "slot in") **child content** written between opening and closing `` tags to any `Component.astro` file.
...
**Refactor** your other pages (`blog.astro` and `about.astro`) so that they use your new `` component to render the common page elements.
...
1. An Astro component (`.astro` file) can function as a:
      ...
      UI component
      layout
      <Option isCorrect>all of the above, because Astro components are so functional! 🏗️
