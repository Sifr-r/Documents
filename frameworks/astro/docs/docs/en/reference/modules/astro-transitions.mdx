...
:::note
This API is compatible with the `` included in `astro:transitions`, but can't be used with native browser MPA routing.
:::
...
Opt in to using view transitions on individual pages by importing and adding the `` routing component to `<head>` on every desired page.
...
## Imports from astro:transitions/client (part 2)
...
## Imports from astro:transitions/client (part 3)
...
## Imports from astro:transitions/client (part 4)
