...
## Imports from astro/config (part 2)
...
**Type:** `(options: EnumFieldInput) => EnumField`
</p>
...
Retrieves a no-op image service. This is useful when your adapter does not support Astro's built-in Sharp image optimization and you want to [use the `` and `` components](/en/guides/images/#astro-components-for-images).
...
## Imports from astro/config (part 3)
