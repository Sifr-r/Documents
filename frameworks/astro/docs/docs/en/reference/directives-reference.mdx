...
You can also use `set:html` on a `` to avoid adding an unnecessary wrapper element. This can be especially useful when fetching HTML from a CMS.
...
`set:html={Promise}` injects a [Response](https://developer.mozilla.org/en-US/docs/Web/API/Response) into an element.
...
## Script & Style Directives (part 2)
...
---
<Katex is:raw>Some conflicting {syntax} here
```
