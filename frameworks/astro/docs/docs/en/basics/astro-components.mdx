...
This pattern is the basis of an [Astro layout component](/en/basics/layouts/): an entire page of HTML content can be “wrapped” with `` tags and sent to the component to render inside of common page elements defined there.
...
To pass multiple HTML elements into a component's `<slot/>` placeholder without a wrapping `<div>`, use the `slot=""` attribute on [Astro's `` component](/en/reference/astro-syntax/#fragments):
