...
In Astro v3.x, projects using the `` component were required to opt-in to handling `submit` events for `form` elements. This was done by passing a `handleForms` prop.

Astro v4.0 handles `submit` events for `form` elements by default when `` are used. The `handleForms` prop has been deprecated and no longer has any effect.
...
## Previously deprecated features now removed (part 2)
