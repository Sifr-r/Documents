...
- content within the [built-in `` component](#code-) (powered by Shiki) in `.astro` files.
- content within the [`` component](#prism-) (powered by Prism) in `.astro` files.
...
These values are passed to the `` component using the `theme`, `lang`, [`embeddedLangs`](#embeddedlangs), [`transformers`](#transformers), and `defaultColor` attributes respectively as props. The `` component will not inherit your `shikiConfig` settings for Markdown code blocks.
...
**Type:** `ShikiTransformer[] | undefined`<br />
<Since v="4.11.0" />
</p>
...
## Components for code blocks (part 2)
...
Then, you can import and use the `` component like any other Astro component, passing a language and the code to render.
...
If you opt to use Prism (either by configuring `markdown.syntaxHighlight: 'prism'` or with the `` component), Astro will apply Prism's CSS classes instead of Shiki's to your code. You will need to bring your own CSS stylesheet for syntax highlighting to appear.
