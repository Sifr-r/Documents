...
3. Import your query function in a `.astro` component. After fetching data, the results from the query can be used in your Astro template.
    ...
    The following example shows fetching metadata from Cosmic `posts` and passing these values as props to a `` component to create a list of blog posts:
...
The example below passes these values to a `` component to display a formatted list of blog posts:
...
## Building a Blog with Astro and Cosmic (part 2)
