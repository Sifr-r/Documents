...
2. Change any [Nuxt or Vue syntax to Astro](#reference-convert-nuxtjs-syntax-to-astro) or to HTML web standards. This includes ``, `:class`, `{{variable}}`, and `v-if`, for example.

## Convert your NuxtJS Project (part 2)
...
## Convert your NuxtJS Project (part 3)
...
Astro does not use any special component for links, although you are welcome to build custom link components. You can then import and use this `` just as you would any other component.
...
## Reference: Convert NuxtJS Syntax to Astro (part 2)
...
## Reference: Convert NuxtJS Syntax to Astro (part 3)
...
## Guided example: See the steps! (part 2)
...
3. Add any needed imports, props and JavaScript
    ...
    - A `` component is imported, and wraps the page templating.
      - Our `head()` Nuxt method is passed to the `` component, which is passed to the `<title>` element as a property.
