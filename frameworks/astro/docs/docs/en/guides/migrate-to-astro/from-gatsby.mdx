...
## Convert your Gatsby Project (part 2)
...
## Convert your Gatsby Project (part 3)
...
You may also wish to reuse code from Gatsby's `src/components/seo.js` to include additional site metadata. Notice that Astro uses neither `` nor `` but instead creates `<head>` directly. You may import and use components, even within `<head>`, to separate and organize your page content.
...
## Convert your Gatsby Project (part 4)
...
Convert any Gatsby `<Link to="">`, `` etc. components to HTML `<a href="">` tags. 
...
Astro does not use any special component for links, although you are welcome to build your own `` component. You can then import and use this `` just as you would any other component.
...
## Reference: Convert to Astro Syntax (part 2)

Convert Gatsby's `` and `` components to [Astro's own image integration components](/en/guides/images/), or to a [standard HTML `<img>` / JSX `<img />`](/en/guides/images/#images-in-ui-framework-components) tag as appropriate in your React components.
...
## Guided example: Gatsby layout to Astro (part 2)
...
- Provide the props via `Astro.props`. (Remember: your Astro templating accesses props from its frontmatter, not passed into a function.) 
    ...
    - Replace Gatsby's `` tags with `<a>` anchor tags.
